...
try {
  if (class_exists('DocumentManager') && method_exists('DocumentManager','search')) {
    ...
    $db = new Database();
    if ($q) {
      $stmt = $db->query("SELECT d.id, d.title, d.file_size as size, d.created_at, u.full_name as uploader FROM documents d LEFT JOIN users u ON u.id = d.uploaded_by WHERE d.company_id = :cid AND d.status = 'active' AND d.title LIKE :q ORDER BY d.created_at DESC LIMIT 100")
             ->bind(':cid', $company_id)
             ->bind(':q', "%{$q}%");
    } else {
      $stmt = $db->query("SELECT d.id, d.title, d.file_size as size, d.created_at, u.full_name as uploader FROM documents d LEFT JOIN users u ON u.id = d.uploaded_by WHERE d.company_id = :cid AND d.status = 'active' ORDER BY d.created_at DESC LIMIT 100")
             ->bind(':cid', $company_id);
    }
    $docs = $stmt->fetchAll();
  }
...
<div class="d-flex justify-content-between align-items-center mb-3">
  ...
  <form class="d-flex" method="get" action="documents.php">
    <input type="text" class="form-control me-2" name="q" value="<?php echo htmlspecialchars($q); ?>" placeholder="Căutare...">
    <button class="btn btn-outline-secondary">Caută</button>
    <?php if($q): ?><a class="btn btn-link" href="documents.php">Reset</a><?php endif; ?>
  </form>
</div>
<div class="card mb-4">
  <div class="card-body">
  ...
  if (function_exists('csrf_field')) { echo csrf_field(); } ?>
      <div class="row g-2 align-items-end">
        <div class="col-md-4">
          <label class="form-label small text-muted">Fișier</label>
          <input type="file" name="document" class="form-control" required>
        </div>
        <div class="col-md-4">
          <label class="form-label small text-muted">Titlu</label>
          <input type="text" name="title" class="form-control" placeholder="Titlu opțional">
        </div>
        <div class="col-md-4">
